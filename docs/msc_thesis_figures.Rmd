...
fig_1_static_chap_1 <- joined %>% 
  filter(climate != "none", chapter %in% c("none", "both", "chap_1"), species=="MAAM") %>% 
  group_by(scenarioId, species, iteration) %>% 
  group_modify(~mutate(.x , baseline = subset(.x, timestep == 2010)$sum_cur, 
                       the_diff = ((sum_cur-baseline)/baseline)*100 )) %>% 
  mutate(unique=paste0(sce, iteration)) %>% 
  ...
  geom_point(aes(group = sce), size=0.1) +
  geom_line(aes(group=unique, linetype=code_run))+
