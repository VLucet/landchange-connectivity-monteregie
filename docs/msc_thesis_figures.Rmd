...
# Set the REPROG_FIGS_ONLY
REPRO_FIGS_ONLY <- Sys.getenv("REPRO_FIGS_ONLY")
if(REPRO_FIGS_ONLY == ""){
  REPRO_FIGS_ONLY <- FALSE
} else {
...
mrc.mont.reproj <- st_transform(mrc.mont, crs = st_crs(mun))
...
# results key
sce_code_vec <- 
  as.vector(t(outer(c("BAU-", "BAUR-", "CP-", "CPR-", "CPTR-"), 
                    c("Hist", "Base", "RCP8"), paste0)))
sce_code_vec_run <- 
  rep(c("BAU", "BAUR", "CP", "CPR", "CPTR"), each=3)
...
```{r roc_curves, eval=!(REPRO_FIGS_ONLY), message=FALSE, warning=FALSE, out.width="100%"}
# Roc curves data
...
hist_plot_1 <- histograms %>% 
  ...
  labs(x = "Flow intensity distribution (log)", 
       y = "")+
  theme(strip.text.y = element_text(angle=360, size=20, hjust = 0), 
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(), 
        axis.text.y.left = element_blank(),
        axis.ticks.y.left = element_blank(), 
...
#### BOTH
```{r hist_plot_3, message=FALSE, warning=FALSE, out.width="100%"}
wes_2_colors = wes_palette("Zissou1", 2, type = "continuous")
hist_plot_3 <- histograms %>% 
  mutate(ts = as.factor(ts)) %>% 
  
  filter(chapter %in% c('both', 'chap_1', 'chap_2')) %>%  
  filter(climate != "baseline") %>% 
  
  ggplot(aes(x=bins, y=n, group=ts)) + 
  geom_density(stat='identity', aes(colour=factor(ts)), alpha=0.3)+
  facet_grid(code~species) +
  # scale_fill_manual(values = wes_2_colors, 
  #                   labels = c("2010", "2100")) +
  scale_color_manual(values = wes_2_colors, 
                     labels = c("2010", "2100")) +
  labs(x = "Flow intensity distribution (log)", 
       y = "") +
  theme(strip.text.y = element_text(angle=360, size=25, hjust = 0), 
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(), 
        axis.text.y.left = element_blank(),
        axis.ticks.y.left = element_blank(),
  ...
  NULL
#hist_plot_2
ggsave("../thesis/figures/hist_both_chap.png", hist_plot_3, width = 18, height = 15)
...
```{r 2010_compare_prep, eval=!(REPRO_FIGS_ONLY), message=FALSE, warning=FALSE, out.width="100%"}
...
```{r 2010_compare_urb, eval=!(REPRO_FIGS_ONLY), message=FALSE, warning=FALSE, out.width="100%"}
...
```{r compare, eval=!(REPRO_FIGS_ONLY), message=FALSE, warning=FALSE, out.width="100%"}
source('../scripts/functions/draw_scenario.R')
mxp <- 1e10000 # 1000
the_dpi = 300
for (scenario in c(39, 42, 45, 48, 51)){
  the_plot <- draw_scenario(scenario, mxp = mxp, sce_folder = "/media/vlucet/backup/results_july/monteregie-conncons-scripted.ssim.output/")
  the_name <- paste0("scenario_", as.character(scenario),"_compare.png")
  ggsave(plot = the_plot,
  filename = paste0("../thesis/figures/",the_name), width = 15, height = 18, dpi = the_dpi)
}
```
...
### Networks
```{r, message=FALSE, warning=FALSE, out.width="100%", fig.cap=""}
...
```{r}
networks %>% 
  filter(scenario != 37, timestep %in% c(2,10)) %>% 
  filter(species %in% c("MAAM", "URAM")) %>% 
  pivot_longer(numNodes:overallECnatal, names_to = "metric", values_to = "values") %>% 
  select (species, iteration, timestep, code_run, climate, metric, values) %>% 
  group_by(species, timestep, code_run, climate, metric) %>% 
  summarise(values = mean(values)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = timestep, values_from = values) %>% # names_glue = "ts_{timestep}_{.value}",
  mutate(percent_change = (ts_10_values - ts_2_values)/ts_2_values) %>% 
  select(-starts_with("ts")) %>% 
  pivot_wider(names_from = metric, values_from = percent_change) %>% 
  mutate(code_run = fct_relevel(code_run, c("BAU", "BAUR", "CP", "CPR", "CPTR"))) %>% 
  arrange(code_run) %>% 
  mutate(climate = fct_relevel(climate, c("baseline", "RCP 8.5")))  %>% 
  arrange(code_run, climate) %>% 
  mutate(scenario = paste(code_run, climate)) %>%
  arrange(code_run, climate, scenario) %>% 
  select(-c(code_run, climate)) %>%
  select(species, scenario, starts_with("overall"), numNodes, numLinks, meanAreaQuality, meanCost) %>% 
  
  gt(rowname_col = "scenario", groupname_col = "species") %>% 
  fmt_percent(everything()) %>% 
  cols_label(
    overallECgap = "EC (gap)", 
    overallECnatal = "EC (natal)", 
    numNodes = "Nodes", 
    numLinks = "Links", 
    meanAreaQuality = "Mean Quality", 
    meanCost = "Mean Cost"
  ) %>% 
  data_color(
    columns = overallECgap:meanAreaQuality, 
    colors = scales::col_bin(palette = as.character(paletteer::paletteer_c("scico::cork", n = 100)), 
                             bins = seq(from =-1, to = 1, by = 0.1))) %>% 
  data_color(
    columns = meanCost, 
    colors = scales::col_bin(palette = as.character(paletteer::paletteer_c("scico::cork", n = 100)), 
                             bins = seq(from =-1, to = 1, by = 0.1), reverse = -1)) -> metrics_table 
metrics_table
gtsave(metrics_table, "../thesis/figures/metrics.png")

```

```{r}
networks_tbl <- networks
# networks_tbl$code_run[networks_tbl$code_run == "BAUR"] <- "\n    BAUR"
# networks_tbl$code_run[networks_tbl$code_run == "CP"] <- "    CP"
# networks_tbl$code_run[networks_tbl$climate == "RCP 8.5"] <- "RCP 8.5"
networks_tbl %>% 
  filter(scenario != 37, timestep %in% c(2,10)) %>% 
  # filter(species %in% c("MAAM", "URAM")) %>% 
  pivot_longer(numNodes:overallECnatal, names_to = "metric", values_to = "values") %>% 
  ...
  summarise(values = mean(values)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = timestep, names_glue = "ts_{timestep}_{.value}", values_from = values) %>% 
  mutate(percent_change = (ts_10_values - ts_2_values)/ts_2_values) %>% 
  select(-starts_with("ts")) %>% 
  ...
  pivot_wider(names_from = "scenario", values_from = "percent_change") %>% 
  
  # select(species, scenario, starts_with("overall"), numNodes, numLinks, meanAreaQuality, meanCost) %>% 
  filter(metric %in% c("overallECgap", "overallECnatal", "numNodes", 
                       "numLinks", "meanAreaQuality")) %>% 
  
  # mutate(metric = fct_relevel(metric, c("overallECgap", "overallECnatal", "numNodes", 
  #                                       "numLinks", "meanAreaQuality"))) %>% 
  left_join(data.frame(metric = c("overallECgap", "overallECnatal", "numNodes", 
                                  "numLinks", "meanAreaQuality"), 
                       metric_name = c("EC (gap)", "EC (natal)", "Nodes", 
                                       "Links", "Quality"))) %>% 
  mutate(metric_name = fct_relevel(metric_name, c("EC (gap)", "EC (natal)", "Nodes", 
                                                  "Links", "Quality"))) %>%
  arrange(metric_name) %>%
  select(-metric) %>% 
  
  gt(rowname_col = "metric_name", groupname_col = "species") %>% 
  fmt_percent(everything(), decimals=0) %>% 
  data_color(
    columns = `BAU baseline`:`CPTR RCP 8.5`, 
    colors = scales::col_bin(palette = as.character(paletteer::paletteer_c("scico::cork", n = 100)), 
                             bins = seq(from =-1, to = 1, by = 0.1))) %>%
  tab_options(data_row.padding = px(1)) %>% 
  cols_width(metric_name~px(89), everything()~px(68)) %>% 
  tab_style(locations = cells_body(), cell_text(size = 12)) -> metrics_table_2 # %>% # 89
# data_color(
#   columns = meanCost, 
#   colors = scales::col_bin(palette = as.character(paletteer::paletteer_c("scico::cork", n = 100)), 
#                            bins = seq(from =-1, to = 1, by = 0.1), reverse = -1)) 

metrics_table_2
gtsave(metrics_table_2, "../thesis/figures/metrics_2.png")
...
URAM <- networks %>% 
  ...
  ungroup() %>% 
  pivot_wider(names_from = timestep, names_glue = "ts_{timestep}_{.value}", values_from = values) %>% 
  mutate(percent_change = (ts_10_values - ts_2_values)/ts_2_values*100) %>% 
  select(-starts_with("ts")) %>% 
  mutate(code_run = fct_relevel(code_run, c("BAU", "BAUR", "CP", "CPR", "CPTR"))) %>% 
  arrange(code_run) %>% 
  mutate(climate = fct_relevel(climate, c("baseline", "RCP 8.5")))  %>% 
  ...
  arrange(metric_name) %>%
  select(-metric) %>% 
  mutate(`% Change` = percent_change, Metric = metric_name, Scenario = scenario) %>% 
  tidyHeatmap::heatmap(Scenario, Metric, `% Change`, .scale="none", cluster_columns = FALSE)
...
mtcars_tidy_nested <- bind_rows(mutate(mtcars_tidy, Year=1990), mutate(mtcars_tidy, Year=2000)) %>% 
  mutate(Value = runif(n = n(), min = 0, max = 10))
mtcars_tidy_nested %>% 
...
HEATMAP_data <- networks %>% 
  ...
  arrange(metric_name) %>%
  select(-metric) %>% 
  mutate(`% Change` = percent_change, `Connectivity Metric` = metric_name, `Species / Scenario` = scenario) %>% 
  select(species, `% Change`, `Connectivity Metric`, `Species / Scenario`)

titles <- expand_grid(left = c("BAU", "BAUR", "CP", "CPR", "CPTR"), right = c("baseline", "RCP 8.5")) %>% mutate(final = paste(left, right)) %>% pull(final) %>% sort()

HEATMAP_data %>% 
  unite( "Species / Scenario", c(`Species / Scenario`, species), remove = F) %>%  group_by(species) %>% 
  tidyHeatmap::heatmap(`Species / Scenario`, `Connectivity Metric`, `% Change`, .scale="none", cluster_columns = FALSE, 
                       cluster_rows = FALSE, palette_value = c("red", "white", "darkGreen"), 
                       rect_gp = grid::gpar(col = "#161616", lwd = 0.1),
                       palette_grouping = list("grey"), row_labels = rep(titles, each = 5), 
                       heatmap_legend_param = list(at = c(-60, 0 , 60))) -> heatmap_networks

zissou_palette_custom <- c("#F21A00", "#E67D00", "#E4B80E",
                           "#FFFFFF",
                           "#9EBE91", "#4EA365", "#315849")

HEATMAP_data %>% 
  unite( "Species / Scenario", c(`Species / Scenario`, species), remove = F) %>%  group_by(species) %>% 
  tidyHeatmap::heatmap(`Species / Scenario`, `Connectivity Metric`, `% Change`, .scale="none", cluster_columns = FALSE, 
                       cluster_rows = FALSE, palette_value = 
                         circlize::colorRamp2(c(-60, -40, -20, 0, 20, 40, 60), 
                                              zissou_palette_custom), 
                       # rect_gp = grid::gpar(col = "#161616", lwd = 0.1),
                       palette_grouping = list("grey"), row_labels = rep(titles, each = 5), 
                       heatmap_legend_param = list(at = c(-60, -40, -20, 0, 20, 40, 60))
  ) -> heatmap_networks

heatmap_networks %>% 
  tidyHeatmap::save_pdf(filename = "../thesis/figures/heatmap_networks.pdf")
...
```{r}
joined %>% 
  filter(climate != "none", chapter %in% c("none", "both", "chap_1", "chap_2")) %>% 
  group_by(scenarioId, species, iteration) %>% 
  group_modify(~mutate(.x , baseline = subset(.x, timestep == 2010)$sum_cur, 
                       the_diff = ((sum_cur-baseline)/baseline)*100 )) %>% 
  ungroup() %>% 
  filter(timestep == 2100) %>%
  group_by(code, species) %>% 
  summarise(the_diff = mean(the_diff)) %>% 
  ungroup() %>% 
  mutate(`% Change` = the_diff, Species = species, Scenario = code) %>% 
  select(Species, `% Change`, Scenario) -> current_heatmap_df

current_heatmap_df %>% 
  # unite( "Scenario", c(Scenario, Species), remove = F) %>%  group_by(Species) %>% 
  tidyHeatmap::heatmap(Scenario, Species, `% Change`, .scale="none", cluster_columns = TRUE, 
                       cluster_rows = TRUE,
                       palette_value = circlize::colorRamp2(c(-15, -10, -5, 0), viridis::magma(4))) -> heatmap_current
```
#### Scenario map
...
corridors <- st_read("../data/raw/workshop/corridors/Key_links_smoothed_RegTables.shp", 
                     quiet = TRUE) %>% sf::st_make_valid() %>% st_buffer(1000) %>% 
  mutate(type = "Corridors")
areas <- st_read("../data/raw/workshop/ensembles/Key_ensembles_smoothed_RegTables.shp", 
                 quiet = TRUE) %>% sf::st_make_valid()%>% 
  mutate(type = "Cores")
background <- raster("../data/land_use/LandUse_mont_aafc_buffered_30by30_2010.tif")
background_cropped <- crop(background, st_transform(areas, st_crs(background)))

background_stars <- stars::st_as_stars(background_cropped) %>% 
  mutate(`Land Use Class` = 
           as.factor(case_when(LandUse_mont_aafc_buffered_30by30_2010 == 1 ~ "Agriculture", 
                               LandUse_mont_aafc_buffered_30by30_2010 %in% c(2,4) ~ "Urban / Roads",
                               LandUse_mont_aafc_buffered_30by30_2010 == 3 ~ "Forest",
                               LandUse_mont_aafc_buffered_30by30_2010 == 5 ~ "Water",
                               LandUse_mont_aafc_buffered_30by30_2010 == 6 ~ "Wetlands")))

tmap_mode("plot")
tmap_options(check.and.fix = TRUE)
test <- tm_shape(background_stars, raster.downsample = FALSE) + 
  tm_raster("Land.Use.Class", palette = c("#F9E79F", # Ag
                                          "#58D68D", # Fo
                                          "#ABB2B9", # Urb / Rd
                                          "#5DADE2", # Wat
                                          "#A569BD"),# Wet
  ...
  tm_shape(areas) + 
  tm_borders(lwd=1, col ="#283747") + 
  tm_fill("type", col = "#196F3D", alpha = 0.3) +
  tm_shape(corridors) + 
  tm_borders(lwd=2, col = "#0B5345", lty = 3) +
  tm_layout(legend.bg.color = "white", legend.bg.alpha = .9, 
            legend.frame = "gray50")
tmap_save(test, "../thesis/figures/corridors_cores.png")
```