...
In order to understand what stakeholders perceive as a conservation priority, and derive conservation scenarios from those perceptions, we conducted a community workshop (a conceptual figure of the methodology is to be found in Appendix 2 under the form of the Workshop participant roadmap). The workshop was approved by McGill University’s Research Ethics Board (see Appendix 2 for the approval letter). The workshop was held on January 22nd 2020 in the public library of Saint-Jean sur Richelieu. \\
...
\begin{enumerate}
  ...
  \item Representatives of the forestry producers union (“producteurs forestiers”)
\end{enumerate}
Although representatives of this last group were invited to the activity, none attended the workshop. \\
...
To ensure that this system is used consistently throughout the workshop, each participant was given a personal folder with their own color-coded stickers and post-its. Participants were instructed to only use the stickers and material that have been personally handed to them. All participants were given a visual aid to remind them of the agenda of the workshop (see Appendix 2). Facilitators were also present at almost every table and helped facilitate the discussion (faciltators were either affiliated to McGill University or to the RMN, and one of them was an independent contractor and workshop professional). They were given a document to help them in this role.\\
...
During our community workshop, participants identified connectivity conservation priorities in Montérégie. Participants quickly built consensus on which key cores areas and corridors should be prioritized for protection or restoration. They mainly identified large forested regions and protected areas as key cores areas. As key corridors, participants were strongly drawn to linear strips of connecting “stepping-stone” patches. This is to be expected as we presented the concepts of linear ecological corridors and stepping-stone habitat to participants at the beginning of the workshop. Participants were rarely drawn to redundant corridor design. When participants were asked what factors would make a link a priority, they emphasized the quality of habitats to be linked, as well as a diversity of constraining and facilitating factors.
...
\caption[Map of "opportunity" and "challenges", subregional tables - summary from workshop results (French)]{Map of "opportunity" and "challenges" resulting from the consensual mapping exercices. During our workshop, the study region was divided in multiple subregions, for each a singular table was set up. For each table, the two elements identified by workshop participants as "oportunity" and "challenges", and which scored the highest in both importance and agreement are identified on the map.}
\label{fig:reg_AC}
\end{figure}
\clearpage
...
\caption[Map of "opportunity" and "challenges", regional tables - summary from workshop results (French)]{Map of "opportunity" and "challenges" resulting from the consensual mapping exercices. During our workshop, the study region was divided in multiple subregions, and two tables were set up for the entire region. For these tables, the elements identified by workshop participants as "oportunity" and "challenges", and which scored the highest in both importance and agreement are identified on the map.}
\label{fig:trans_AC}
\end{figure}
\clearpage
...
\caption[Map of priority areas and links, subregional tables - summary from workshop results (French)]{Map of priority areas and links esulting from the consensual mapping exercices.During our workshop, the study region was divided in multiple subregions, for each a singular table was set up. For each tables, different sets of priority areas (Red) and links (green) were identified and mapped.
}
\label{fig:reg}
\end{figure}
\clearpage
...
\caption[Map of priority areas and links, regional tables - summary from workshop results (French)]{Map of priority areas and links esulting from the consensual mapping exercices.During our workshop, the study region was divided in multiple subregions, and two tables were set up for the entire region. For these tables, different sets of priority areas (Red) and links (green) were identified and mapped.
}
\label{fig:trans}
\end{figure}
\clearpage
...
\begin{figure}[h!]
\makebox[\textwidth]{
\includegraphics[width=1.2\textwidth]{thesis/figures/bar_chap2.png}
}
 \caption[Land use class fequencies in 2010 and 2100 for the BAU and Conservation scenarios]{Land use class fequencies (in hectares) for Forest, Agriculture and Urban, in 2010 (observed from the land use data) and 2100 (predicted by the land use change model) for the BAU (Business As Usual) and Conservation scenarios (Note: these predictions are done under baseline climate scenario). Ref = Reforestation, Corrs Protection = Corridors Protection.}
\label{fig:bar_chap2}
\end{figure}
...
\makebox[\textwidth]{
  ...
  \caption[Linear graph of change in mean flow (decrease in \% of the baseline 2010 flow) between 2010 (observed) and 2100 (predicted), comparing BAU scenario with conservation scenarios]{Linear graph of change in mean flow (decrease in \% of the baseline 2010 flow) between 2010 (observed) and 2100 (predicted). This compares the BAU (Business As Usual) scenario with conservation scenarios, facetted by species. Note that the line type represents the different climate scenarios so as to represent all scenario combinations (R = Reforestation, R(T) = Reforestation Targeted, CorPr = Corridors Protected).}
...
\makebox[\textwidth]{
  ...
  \caption[Radar graph of change in mean flow (decrease in \% of the baseline 2010 flow) between 2010 (observed) and 2100 (predicted), comparing BAU scenario with conservation scenarios]{Radar graph of change in mean flow (decrease in \% of the baseline 2010 flow) between 2010 (observed) and 2100 (predicted). This compares the BAU (Business As Usual) scenario with conservation scenarios, for all 5 species. Note that the line color represent species and that each side of the graph is a different scenario combination (R = Reforestation, R(T) = Reforestation Targeted, CorPr = Corridors Protected).}
...
\begin{figure}[h!]
\makebox[\textwidth]{
  \includegraphics[width=1.25\textwidth]{thesis/figures/hist_chap2.png}
}
  \caption[Histograms of flow values for all species and scenarios, comparing timesteps 2010 (observed) with 2100 (predicted)]{Histograms of flow values for the years 2010 \& 2100. This compares the observed values for 2010 (in blue) with the values of the model output in 2100 (in red). Results are facetted by species and scenario combination.}
 \label{fig:hist_2}
\end{figure}
...
\makebox[\textwidth]{
  \includegraphics[width=1.3\textwidth]{thesis/figures/surf_chap2.png}
   ...
   \caption[Linear graph of change in the number of features (decrease in \% of the baseline 2010 numbers) between 2010 (observed) and 2100 (predicted), comparing BAU scenario with conservation scenarios]{Linear graph of change in the number of features (decrease in \% of the baseline 2010 numbers) between 2010 (observed) and 2100 (predicted). This compares the BAU (Business As Usual) scenario with conservation scenarios, facetted by species. Note that the line type represents the different climate scenarios so as to represent all scenario combinations (R = Reforestation, R(T) = Reforestation Targeted, CorPr = Corridors Protected).}
...
\makebox[\textwidth]{
 ...
 \caption[Radar graph of change in number of features (decrease in \% of the baseline 2010 numbers) between 2010 (observed) and 2100 (predicted), comparing BAU scenario with conservation scenarios]{Radar graph of change in number of features (decrease in \% of the baseline 2010 numbers) between 2010 (observed) and 2100 (predicted). This compares the BAU (Business As Usual) scenario with conservation scenarios, for all 5 species. Note that the line color represent species and that each side of the graph is a different scenario combination (R = Reforestation, R(T) = Reforestation Targeted, CorPr = Corridors Protected).}
